...
function convertToFlaggedWords(analysisResult) {
    const flaggedWords = [];
    // Check if analysisResult and biased_clauses exist
    if (!analysisResult || !analysisResult.biased_clauses || !Array.isArray(analysisResult.biased_clauses)) {
        return flaggedWords;
    }
    analysisResult.biased_clauses.forEach((clause)=>{
        if (clause && clause.detected_biases && clause.detected_biases.length > 0) {
            // Extract words from the clause that might be contributing to bias
            const words = clause.clause ? clause.clause.split(/\s+/) : [];
            // For each detected bias, try to identify relevant words
            clause.detected_biases.forEach((bias)=>{
                if (bias && bias.bias_type) {
                    const biasKeywords = getBiasKeywords(bias.bias_type);
                    words.forEach((word)=>{
                        const cleanWord = word.toLowerCase().replace(/[^\w]/g, "");
                        // Check if this word is likely contributing to the bias
                        if (biasKeywords.some((keyword)=>cleanWord.includes(keyword) || keyword.includes(cleanWord))) {
                            // Find the word's position in the original text
                            const wordIndex = analysisResult.original_text ? analysisResult.original_text.toLowerCase().indexOf(cleanWord) : -1;
                            if (wordIndex !== -1) {
                                flaggedWords.push({
                                    word: word,
                                    ...
                                    suggestions: generateSuggestions(word, bias.bias_type),
    ...
    // Remove duplicates based on word and position
    const uniqueFlaggedWords = flaggedWords.filter((word, index, self)=>index === self.findIndex((w)=>w.word === word.word && w.startIndex === word.startIndex));
    return uniqueFlaggedWords;
}
/**
 * Get keywords associated with each bias type
 */ function getBiasKeywords(biasType) {
    if (!biasType) return [];
    const keywordMap = {
        racial: [
            "race",
    ...
    return keywordMap[biasType] || [];
}
/**
 * Generate alternative word suggestions based on bias type
 */ function generateSuggestions(word, biasType) {
    if (!word || !biasType) return [];
    const suggestions = [];
    // Bias-specific replacements
    const replacements = {
        educational: {
            stupid: [
                "unwise",
    ...
    // Check bias-specific replacements first
    if (replacements[biasType]?.[lowerWord]) {
        suggestions.push(...replacements[biasType][lowerWord].map((replacement)=>({
                word: replacement,
                confidence: 0.8,
                reason: `More inclusive alternative for ${biasType} bias`
            })));
    }
    ...
    if (generalReplacements[lowerWord]) {
        suggestions.push(...generalReplacements[lowerWord].map((replacement)=>({
                word: replacement,
                confidence: 0.7,
                reason: "More inclusive alternative"
            })));
    }
    ...
    if (suggestions.length === 0) {
        suggestions.push({
            ...
            confidence: 0.6,
            reason: `Consider using more inclusive language (${biasType} bias detected)`
        });
    }
    return suggestions;
}
...
function IssuesPanel({ analysisResult, isAnalyzing, onWordReplace, onIssueClick, text, onClear, onAnalyze, maxLength, error, validationErrors = [] }) {
...
function BiasDetectionApp({ initialText = '', onBack }) {
    ...
    const handleValidationChange = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useCallback"])((isValid, errors)=>{
        setValidationErrors(errors);
    }, []);
...
//# sourceMappingURL=%5Broot-of-the-server%5D__2670b33d._.js.map